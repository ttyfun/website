...
function onDraw(ctx) {
  ...
  ctx.save();
  ctx.translate(headPoint.x-80, headPoint.y-180);
  // ctx.rotate(angle);
  // ctx.scale(peaking ? 1.2 : 1, peaking ? 1.2 : eyeHeight.s);

  if ( stress > 40 /* && stress < 90 */ ) {
    stress += 1;
    document.getElementById('shake').className = "hidden";
...
function setPositions() {
  ...
  mousePos.x += (targetMousePos.x - mousePos.x);
